			...
			if (options.scales) {
				items = items.concat(
					(options.scales.xAxes || []).map(function(xAxisOptions) {
						return {options: xAxisOptions, dtype: 'categories'};
					}),
			...
			if (key === 'xAxes' || key === 'yAxes') {
				...
				if (base.hasOwnProperty(key)) {
					helpers.each(value, function(valueObj, index) {
						var axisType = helpers.getValueOrDefault(valueObj.type, key === 'xAxes' ? 'categories' : 'linear');
...
module.exports = function(Chart) {
	...
	Chart.scaleService.registerScaleType('categories', DatasetScale, defaultConfig);
